...
| `/novel-dna distill` 或 `蒸馏文风` | **文风蒸馏** | 从 20+ 名家章节中提取 L1 语言、L2 结构、L3-L5 认知框架，输出至 `文风DNA/` 目录 |
| `/novel-dna scan` 或 `网文扫榜` | **市场洞察** | 扫描起点/番茄/知乎/晋江当前风口题材、热门金手指与过签偏好 |
| `/novel-dna outline` 或 `帮我开书` | **商业大纲** | 在 `设定集/` 中初始化世界观、战力体系、角色档案、分卷大纲与黄金三章细纲 |
| `/novel-dna import` 或 `老书接续` | **老书逆向** | 导入已有 10~100+ 章历史正文，逆向提取当前人物状态快照与伏笔矩阵，无缝续写 |
| `/novel-dna write` 或 `写第N章` | **DNA正文** | 强制读取 Writing DNA + 细纲，驱动多视角镜头与权力博弈对白生成正文至 `正文章节/` |
...
| `/novel-dna cover` 或 `生成封面` | **封面设计** | 在 `封面/` 目录生成符合番茄 3:4 / 起点 2:3 / 知乎等平台调性的 AI 提示词与封面 |
...
* **输入**：`语料库/原始章节/` 目录下的 20~30 篇目标作者小说章节（`.md` 或 `.txt`）。
* **执行步骤**：
  1. 为每篇语料在 `语料库/元数据/` 创建对应的 JSON 元数据，用于分类标注；
  2. 运行 `scripts/extract_style_stats.py` 计算平均句长、短句比、长句比、逗号密度、对话占比；
  3. 深度提炼 6 层特征输出至 `文风DNA/` 目录：
     * `文风DNA/语言DNA.md`
     * `文风DNA/文章结构模板.md`
     * `文风DNA/写作视角与认知框架.md`
     * `文风DNA/Writing-DNA.md`（总入口）。
...
* **执行步骤**：
  ...
  3. 在 `设定集/` 目录下初始化核心资产：
     ...
     * `设定集/03_分卷主线与黄金三章细纲.md`（长篇）或 `设定集/短篇_故事核与反转架构.md`（短篇）
     * `设定集/04_伏笔与状态追踪表.md`（初始化伏笔矩阵与主角状态快照）。
...
* **执行步骤**：
  ...
  2. 梳理前文未结悬念与伏笔，自动在 `设定集/` 生成 `02_核心角色档案与声纹表.md` 与 `04_伏笔与状态追踪表.md`；
...
* **前置读取协议（强制，缺一不可）**：
  1. 通读当前项目的 `文风DNA/Writing-DNA.md` 与 `文风DNA/语言DNA.md`；
  2. 从 `语料库/原始章节/` 抽读 3~5 篇原文校准句式呼吸感；
  3. 读取 `设定集/03_分卷主线与黄金三章细纲.md`、`设定集/04_伏笔与状态追踪表.md` 与近 3 章滚动摘要；
  4. 读取 `设定集/02_核心角色档案与声纹表.md` 中出场角色的对白声纹。
* **正文输出铁律**：
  * **场景功能化**：每节必须改变风险/信息/关系/资源/抉择/行动/读者理解中至少一项；
  * **对白权力博弈与潜台词**：掌控者话短（≤10字），被压制方话长；对白暗藏私密议程，杜绝科普嘴；
  * **情绪弧线匹配**：根据章节定位严密执行 V形、倒V、W形、递进、延迟满足或急转弧线；
  * **结尾钩子**：章末留下悬念、危机或利益预期。
* **输出目标**：正文保存至 `正文章节/第NNN章_章节名.md`。
* **正文生成后强制执行（不可跳过）**：
  1. **刷新主角状态快照**（境界、坐标、道具消耗、伤势）；
  2. **伏笔登记/回收**（埋新伏笔在 `设定集/04表` 登 `FB-xxx`，回收改 `Closed`）；
...
* **输出**：在 `封面/` 目录生成符合番茄 3:4 大头贴、起点 2:3 电影感、知乎极简海报等平台调性的专业 AI 提示词与设计图。
